...
| `transfer/mooncake/deferred_delete.py` | _(removed in verl)_ | TorchSpec stopped using `DeferredDeleteManager` (commit 619ad48); verl follows — `remove_eagle3_tensors()` does immediate `batch_remove(force=True)` with retries. |
